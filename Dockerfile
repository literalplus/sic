...
# jdk.unsupported: For Objenesis when there is no default constructor https://github.com/easymock/objenesis/issues/70
...
COPY --from=build /home/gradle/build/libs/sic.jar /sic.jar

EXPOSE 8080
ENTRYPOINT ["/opt/java/openjdk/bin/java", "-jar"]
CMD ["-XX:+UseContainerSupport", "-XX:MaxRAMPercentage=80.0", "/sic.jar"]
